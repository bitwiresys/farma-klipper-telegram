...
function loadFile(path, callback /* function(contents) */) {
  $.get(path, null, callback, 'text').error(function () {
    error();
  });
}
...
function installMessageBridge() {
  ...
  window.addEventListener('message', function (event) {
    try {
      ...
      if (data.type !== 'gcode-viewer:set-gcode') return;
      if (typeof data.gcode !== 'string') return;
      openGCodeFromText(data.gcode);
    } catch (e) {
      // ignore
    }
  });
...
$(function () {
  if (!Modernizr.webgl) {
    alert(
      'Sorry, you need a WebGL capable browser to use this.\n\nGet the latest Chrome or FireFox.',
    );
    return;
  }
  ...
  if (!Modernizr.localstorage) {
    alert(
      "Man, your browser is ancient. I can't work with this. Please upgrade.",
    );
    return;
  }
