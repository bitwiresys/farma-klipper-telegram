function createObjectFromGCode(gcode) {
  ...
  function addSegment(p1, p2) {
    ...
    geometry.vertices.push(
      new THREE.Vertex(new THREE.Vector3(p1.x, p1.y, p1.z)),
    );
    geometry.vertices.push(
      new THREE.Vertex(new THREE.Vector3(p2.x, p2.y, p2.z)),
    );
  ...
  var parser = new GCodeParser({
    G1: function (args, line) {
      ...
      if (delta(lastLine.e, newLine.e) > 0) {
        ...
        if (layer == undefined || newLine.z != layer.z) newLayer(newLine);
      }
  ...
  for (var lid in layers) {
    ...
    for (var tid in layer.type) {
      ...
      object.add(
        new THREE.Line(type.geometry, type.material, THREE.LinePieces),
      );
    }
  }
