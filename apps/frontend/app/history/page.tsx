...
import type { PrintHistoryDto, PrinterDto } from '@farma/shared';
...
export default function HistoryPage() {
  ...
  const load = async () => {
    ...
    const h = await apiRequest<{ history: PrintHistoryDto[] }>(`/api/history?${qs.toString()}`, { token });
      ...
      {token && (
        <>
          <div className="mt-3 flex gap-2">
            <select className="w-full rounded bg-slate-950 p-2 text-xs" value={status} onChange={(e) => setStatus(e.target.value as StatusFilter)}>
          ...
          <div className="mt-3 space-y-2">
            {history.map((h) => (
              <div key={h.id} className="rounded border border-slate-800 bg-slate-900/40 p-3 text-xs">
                ...
                <div className="mt-1 text-slate-400">printer: {printerNameById.get(h.printerId) ?? h.printerId}</div>
                <div className="mt-1 text-slate-400">startedAt: {h.startedAt}</div>
                {h.endedAt && <div className="mt-1 text-slate-400">endedAt: {h.endedAt}</div>}
                {h.errorMessage && <div className="mt-2 break-all text-red-300">{h.errorMessage}</div>}
              </div>
            ))}
